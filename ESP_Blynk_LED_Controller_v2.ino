...
void setup()
{
  ...
  timer.setInterval(500L, checkSchedule);    // check every second if fade should start
...
void checkTemp()
{
  ...
  }else
  {
    ...
    for (i = 0; i < numCh; i = i + 1)
    {
      if(LEDsettings[i].currentPWM > maxCurrentPWM)
      {
        maxCurrentPWM = LEDsettings[i].currentPWM;
      }
      if(LEDsettings[i].maxPWM > maxSetPWM)
      {
        maxSetPWM = LEDsettings[i].maxPWM;
      }
    }
